...
namespace DCI {
  Int Interface::InteriorPointRestoration () {
    ...
    for (int i = 0; i < numUpper; i++) {
      int j = nvar + nconI + i;
      vertID.rhs[j] = upper[i] - slack[upperIndex[i]] - sigma*vertMu/multUpper[i] - rpx[upperIndex[i]];
    }
    for (int i = 0; i < numLower; i++) {
      int j = nvar + nconI + numUpper + i;
      vertID.rhs[j] = slack[lowerIndex[i]] - lower[i] - sigma*vertMu/multLower[i] + rpx[lowerIndex[i]];
    }
